...
def charger_coeffs_phalange(doigt) :
	list_coeffs_phalange = []
	for j in range(2) :
		list_coeffs_phalange.append(load_calibration_from_csv(doigt + str(j+1)))
	return list_coeffs_phalange


def charger_coeffs_doigts() :

	list_coeffs_doigts = dict()
	for i in range(1) :
		list_coeffs_doigts[index_to_arti(i,0)],list_coeffs_doigts[index_to_arti(i,1)] = charger_coeffs_phalange(str(i+1))
	return list_coeffs_doigts


def index_to_arti(i, j):
