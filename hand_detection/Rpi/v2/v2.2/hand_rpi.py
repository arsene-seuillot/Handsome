...
#----- INITIALISATION DU FICHIER TEMPORAIRE POUR L'ENVOI DES DONNÉES -----#
...
# Ce n'est pas très agréable de faire les tests d'angles avec l'affichage de tout le dictionnaire, 
# donc on peut décider d'afficher les angles d'un seul doigt pour la vérification.
if len(sys.argv) < 2:
    sys.stderr.write("Vous pouvez choisir un doigt en paramètre. L'index a été choisi par défaut\n")
    sys.stderr.flush()
    param = "index"
else:
    param = sys.argv[1]
...
def update_frame():
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            ###### ENVOYER LES DONNÉES ######
            sys.stderr.write("attente de lecture ...\n")
            sys.stderr.flush()
            for line in sys.stdin :
            	sys.stderr.write("signal ok pour envoie\n")
            	sys.stderr.flush()
            	break
            #while os.path.getsize(temp_file_path) != 0 : pass
            #sys.stderr.write("attente de lecture ...\n")
            #sys.stderr.flush()
  
            with open(temp_file_path,"wb") as file:
            	pickle.dump(finger_angles, file)
            	
            sys.stdout.write("c'est bon\n")
            sys.stdout.flush()
